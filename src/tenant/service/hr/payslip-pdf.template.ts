...
export const buildPayslipPdfHtml = (
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8" /><title>Employee Payslip ${text(payslip.payslipNumber)}</title><style>
    ...
    html,body { margin:0; padding:0; background:var(--light-bg); font-family:Arial,sans-serif; color:var(--foreground); } body { width:194mm; }
    ...
    table { width:100%; border-collapse:collapse; table-layout:fixed; } th { padding:12px 16px; text-align:left; background:var(--light-bg); font-size:11px; text-transform:uppercase; color:var(--primary); }
    ...
    <div class="table-wrap"><div class="table-title">Earnings &amp; Deductions</div><table><colgroup><col style="width:6%" /><col style="width:24%" /><col style="width:12%" /><col style="width:14%" /><col style="width:18%" /><col style="width:13%" /><col style="width:13%" /></colgroup><thead><tr><th class="center">#</th><th>Component</th><th>Code</th><th>Type</th><th>Calculation</th><th class="number">Value</th><th class="number">Amount</th></tr></thead><tbody>${lineRows}</tbody>${totalRow}</table></div>
